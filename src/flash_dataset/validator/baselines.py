...
class PartitionSnapshot:
    ...
    Args:
        ...
        dataset_entries: One readable parquet entry per required dataset.
    ...
    dataset_entries: tuple[FileIndexEntry, ...]
    ...
    def entry_for_dataset(self, dataset: str) -> FileIndexEntry | None:
        """Return the indexed parquet metadata for one dataset when present."""
...
def collect_partition_snapshot(
    ...
    return PartitionSnapshot(
        ...
        dataset_entries=tuple(
            sorted(entries_by_dataset.values(), key=lambda entry: entry.dataset)
        ),
...
def run_baseline_checks(
    snapshots: Sequence[PartitionSnapshot], result: ValidationResult
) -> None:
    """Emit cross-partition baseline metrics and anomaly warnings."""
    for snapshot in sorted(snapshots, key=lambda item: item.partition):
        ...
        if not _is_drift_peer(snapshot):
            continue
        _add_dataset_physical_drift_warnings(snapshot, snapshots, result)
        _add_transaction_type_mix_drift_warning(snapshot, snapshots, result)


def _add_partition_metrics(
    snapshot: PartitionSnapshot, result: ValidationResult
) -> None:
    for entry in snapshot.dataset_entries:
        result.add(
            make_finding(
                ...
                example={
                    ...
                    "compression_ratio": _round_float(entry.compression_ratio()),
                },
            )
        )
...
def _add_dataset_physical_drift_warnings(
    ...
    for entry in snapshot.dataset_entries:
        ...
        for metric_name, metric_value in _physical_metrics(entry).items():
            ...
            peer_values = [
                ...
                for peer_metric in [
                    _physical_metric_value(peer, entry.dataset, metric_name)
                ]
...
def _physical_metric_value(
    ...
    entry = snapshot.entry_for_dataset(dataset)
    if entry is None:
        return None
    return _physical_metrics(entry)[metric_name]


def _physical_metrics(entry: FileIndexEntry) -> dict[str, float | None]:
    return {
        "row_count": float(entry.row_count),
        "file_size_bytes": float(entry.file_size_bytes),
        "row_groups": float(entry.row_groups),
